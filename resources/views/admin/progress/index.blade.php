            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <div class="d-flex">
                            <select class="custom-select mr-2" id="activities">
                                <option value="">Filtrar...</option>
                                @php $rq=(request()->get('k'))?decrypt(request()->get('k')):'' @endphp
                                @foreach ($activities as $item)
                                    <option @if ($item->id==$rq) selected @endif value="{{ encrypt($item->id) }}">{{ $item->name }}</option>
                                @endforeach
                            </select> 
                            <a class="btn btn-success" href="{{ route('progress.create') }}">
                                <span class="icon">
                        ...
                        <table class="table table-hover table-sm table-bordered table-responsive activities">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @if(Auth::user()->role!=0)<td>{{ $progres->feedback }}</td>@endif
                                    ...
                                    <td class="{{ $style }}">
                                        <a class="btn btn-primary text-white btn-sm" href="{{ route('progress.edit', $progres->id) }}" data-toggle="tooltip" data-placement="top" title="Editar">
...
$('document').ready(function(){
    ...
    $('#activities').change(function(){
        $val = $(this).val();
        $link = "{{ route('progress.index') }}";
        if($val)
        {
            window.location.href = $link + "?k=" + $val;
        }
